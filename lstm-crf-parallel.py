...
class BiLSTM_CRF_MODIFY(nn.Module):
    ...
    def __init__(self, tag_to_ix, embedding_dim, hidden_dim):
        ...
        for ix_now in self.tag_to_ix.keys():
            for ix_next in self.tag_to_ix.keys():
                if ix_now[0] == 'O' and ix_next[0] == 'I':
                    self.transitions.data[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.
                elif ix_now[0] in ['B', 'I'] and ix_next[0] == 'I' and ix_now[1:] != ix_next[1:]:
                    self.transitions.data[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.
                elif ix_now[0] == tag_to_ix[START_TAG] and ix_next[0] == 'I':
                    self.transitions.data[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.
    ...
    def restrict_transMat(self):
        for ix_now in self.tag_to_ix.keys():
            for ix_next in self.tag_to_ix.keys():
                if ix_now[0] == 'O' and ix_next[0] == 'I':
                    self.transitions[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.
                elif ix_now[0] in ['B', 'I'] and ix_next[0] == 'I' and ix_now[1:] != ix_next[1:]:
                    self.transitions[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.
                elif ix_now[0] == self.tag_to_ix[START_TAG] and ix_next[0] == 'I':
                    self.transitions[self.tag_to_ix[ix_next]][self.tag_to_ix[ix_now]] = -10000.

    def forward(self, embeds, test_mode=False):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        lstm_feats = self._get_lstm_features(embeds)

        # Find the best path, given the features.
        # begin=time.time()
        score, tag_seq = self._viterbi_decode_new(lstm_feats, test_mode=test_mode)
        # print('viterbi time consume: %f' %(time.time()-begin))
        return score, tag_seq
